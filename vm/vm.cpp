...
void VM::init_stack() noexcept {
	DAVM_BP(m_context) = *DAVM_CAST(register_t*, &*(m_memory.end() - 16));
...
int VM::one_step() noexcept {
	...
	const word_t rd = (code & 0x0F80) >> 7; // Bit 11 - 7 is rd
	switch(code & 0x7F) { // Bit 6 - 0 is opcode
	case I_G_ARITH: {
		const word_t op2 = code >> 22; // Bit 31 - 22 is op2
		DA_IF_UNLIKELY(op2 >= CNT_I_ARITH) {
			return 2;
		}
		const word_t ra = (code & 0x01F000) >> 12; // Bit 16 - 12 is ra
		const word_t rb = (code & 0x4E0000) >> 17; // Bit 21 - 17 is rb
		asm_table_arith[op2](m_context, rd, ra, rb);
		return 0;
	}
	case I_G_SL: {
		const word_t	  op2 = (code & 0x0E0000) >> 17; // Bit 19 - 17 is op2
		const word_t	  ra  = (code & 0x01F000) >> 12; // Bit 16 - 12 is ra
	...
	case I_G_IMM: {
		...
		DA_IF_UNLIKELY(op2 == I_G_IMM_SHIFT) {
			const word_t op3 = (code & 0x700000) >> 20; // Bit 21 - 20 is op3
			DA_IF_UNLIKELY(op3 >= CNT_I_IMM_SHIFT) {
				return 2;
			}
